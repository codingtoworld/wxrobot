...
class WXBot:
    ...
    def __init__(self,threadId="0"):
        ...
        #文件缓存目录
        self.temp_pwd = os.path.join(os.getcwd(), 'data/' + self.threadId).decode('GBK')
        if os.path.exists(self.temp_pwd + '/voices') == False:
            os.makedirs(self.temp_pwd + '/voices')
        if os.path.exists(self.temp_pwd + '/images') == False:
            os.makedirs(self.temp_pwd + '/images')
        if os.path.exists(self.temp_pwd + '/plugin_config') == False:
            os.makedirs(self.temp_pwd + '/plugin_config')
        if os.path.isfile(self.temp_pwd + '/wxqr.png'):
            os.unlink(self.temp_pwd + '/wxqr.png')

        self.session = SafeSession()
        self.session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:54.0) Gecko/20100101 Firefox/54.0'})
        self.conf = {'qr': 'png'}
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            msg_content['img'] = b64encode(self.session.get(msg_content['data']).content) #.encode('hex')
            if self.DEBUG:
            ...
            msg_content['voice'] = b64encode(self.session.get(msg_content['data']).content) #.encode('hex')
            if self.DEBUG:
    ...
    def handle_msg(self, r):
        ...
        for msg in r['AddMsgList']:
            ...
            message = {'msg_type_id': msg_type_id,
                       ...
                       'content': content,
                       'to_user': {"id":msg['ToUserName'],"name":self.get_contact_prefer_name(self.get_contact_name(msg['ToUserName']))},
                       'from_user': user}
    ...
    def proc_msg(self):
        ...
        while True:
            ...
            try:
                ...
                if retcode == '1102':# 1102 后台session 丢失，无法发送消息了，但是还可以接收消息
                    self.send_msg_by_uid(u'微信机器人会话过期，自动退出登录！')
                    break
                elif retcode == '1100':  # 机器人主动登出
                    break
                elif retcode == '1101':  # Web微信登入，手机登出，或是微信客户端登入，微信机器退出登录！
                    break
                elif retcode == '0':
                    if selector == '2':  # 有新消息
                        r = self.sync()
    ...
    def upload_media(self, fpath, is_img=False):
        if not os.path.exists(fpath):
            ...
            return None
        url_1 = '%s/webwxuploadmedia?f=json' % self.file_host
    ...
    def gen_qr_code(self, qr_file_path):
        ...
        self.conf['qr'] = 'png' if sys.platform.startswith('win') else 'tty'
    ...
    def wait4login(self):
        ...
        while retry_time > 0:
            ...
            if code == SCANED:
                ...
                for indexUrl, detailedUrl in WX_URLS_RES:
                    fileUrl, syncUrl = ['https://%s/cgi-bin/mmwebwx-bin' % url for url in detailedUrl]
                    if indexUrl == self.base_host:
                        self.file_host, self.sync_host = fileUrl, syncUrl
                        break
                else:
                    self.file_host = self.sync_host = self.base_uri
    ...
    def sync_check(self):
        ...
        params = {
            ...
            'uin': self.uin,
            'skey': self.skey,
            'deviceid': 'e' + repr(random.random())[2:17], #self.device_id,
            'synckey': self.sync_key_str,
            '_': _t-1024000,
        }
        url = self.sync_host + '/synccheck?' + urllib.urlencode(params)
        try:
    ...
    def get_voice(self, msgid):
        ...
        data = r.content
        fn = 'voice_' + msgid + '.mp3'
        with open(os.path.join(self.temp_pwd,'voices',fn), 'wb') as f:
            f.write(data)
        return fn
